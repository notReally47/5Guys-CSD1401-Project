...
void world_camera(float cellSize, float cellAlign, int playerRow, int playerCol, int face, int cameratoggle) {
	...
	int xoffset = (int)(playerCol - Offset[2].x);		// offset to center the camera to have the player to be in the middle of the screen
	int yoffset = (int)(playerRow - Offset[2].y);
	if (cameratoggle == (toggled + 1))
